...
process.stdin.on('end', async () => {
  ...
  const result = { status: 'completed', summary: 'codex-cli-probe completed', changedFiles: [] };
  await writeFile(output, `${JSON.stringify(result)}\n`, 'utf8');
  process.stdout.write(`${JSON.stringify({ type: 'thread.started', thread_id: 'probe-thread', temp: process.env.TEMP })}\n`);
