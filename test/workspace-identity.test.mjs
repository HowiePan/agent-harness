...
test('a Run pins a verified linked worktree through Authority and Dispatch', async t => {
  ...
  const started = await harness.startRun({ projectId: 'project', runId: 'run', profileId: 'feature-delivery', executionWorkspaceRoot: repository.worktreeRoot, features: [{ id: 'one', executionClass: 'agent-reasoning', acceptance: ['done'], dependsOn: [], allowedPaths: ['worktree.txt'], metadata: {} }], executionAuthorizationEvidence: { explicitUnattended: true } }, command());
