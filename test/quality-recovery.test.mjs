...
test('live hard recovery requires a verified Capsule and creates its policy resolution without a second user approval', async t => {
  const fixture = await makeFixture({ extensions: [legacyCompatibilityExtension] }); t.after(() => fixture.cleanup());
  ...
  const verified = await fixture.harness.recovery.verifyCapsule(capsule.root, { projectId: fixture.projectId, runId: 'run' });
  const recovered = await fixture.harness.recovery.hardRecover({ projectId: fixture.projectId, runId: 'run', verificationRef: verified.verification.verificationRef }, { expectedRevision: state.revision, commandId: 'automatic-resolution' });
  assert.equal(recovered.state.epoch, 2);
  assert.match(recovered.state.recoveryArchives.at(-1).recoveryResolutionRef, /^evidence:/);
  assert.equal(recovered.state.recoveryArchives.at(-1).authorityDecisionId, null);
  const repeated = await fixture.harness.recovery.hardRecover({ projectId: fixture.projectId, runId: 'run', verificationRef: verified.verification.verificationRef }, { expectedRevision: state.revision, commandId: 'automatic-resolution' });
  assert.equal(repeated.reused, true);
});

test('automatic hard recovery obeys explicit Project effect policy without asking for a mechanism choice', async t => {
  const fixture = await makeFixture({
    extensions: [legacyCompatibilityExtension],
    policy: { recovery: { automaticLineageResolution: true, automaticOrdinaryResume: true, automaticVerifiedHardRecovery: false, preserveSupersededRuns: true } },
  });
  t.after(() => fixture.cleanup());
  const state = await startRun(fixture);
  const legacyRoot = resolve(fixture.root, 'legacy-policy-denied');
  await mkdir(legacyRoot, { recursive: true });
  await writeFile(resolve(legacyRoot, 'state.json'), '{}');
  const capsule = await fixture.harness.recovery.createCapsule({ importerId: 'cardworld-t1-t2-importer', legacyRoot, capsuleId: 'policy-denied', commandId: 'create-policy-denied' });
  const verified = await fixture.harness.recovery.verifyCapsule(capsule.root, { projectId: fixture.projectId, runId: 'run' });
  await assert.rejects(
    () => fixture.harness.recovery.hardRecover({ projectId: fixture.projectId, runId: 'run', verificationRef: verified.verification.verificationRef }, { expectedRevision: state.revision, commandId: 'recover-policy-denied' }),
    error => error.code === 'HARD_RECOVERY_POLICY_DENIED',
  );
});

test('policy-generated hard recovery resolution rejects expiry and Authority mismatch', async t => {
  for (const [suffix, expiresAt, revisionOffset, code] of [
    ['expired-resolution', new Date(Date.now() - 60_000).toISOString(), 0, 'RECOVERY_RESOLUTION_EXPIRED'],
    ['wrong-revision', new Date(Date.now() + 60_000).toISOString(), 1, 'RECOVERY_RESOLUTION_CONTEXT_MISMATCH'],
  ]) {
    await t.test(suffix, async t => {
      ...
      await mkdir(legacyRoot, { recursive: true });
      await writeFile(resolve(legacyRoot, 'state.json'), '{}');
      const capsule = await fixture.harness.recovery.createCapsule({ importerId: 'cardworld-t1-t2-importer', legacyRoot, capsuleId: suffix, commandId: `create-${suffix}` });
      const verified = await fixture.harness.recovery.verifyCapsule(capsule.root, { projectId: fixture.projectId, runId: 'run' });
      const resolution = sealRecoveryResolution({
        protocolVersion: '1.0', kind: 'recovery-resolution', authorityBasis: 'explicit-recover-command',
        projectId: fixture.projectId, runId: 'run', logicalTaskKey: null, planDigest: null,
        verificationRef: verified.verification.verificationRef, authorityEpoch: state.epoch, authorityGeneration: state.generation,
        expectedRevision: state.revision + revisionOffset, targetEpoch: state.epoch + 1, action: 'hard-recovery', reasonCode: 'TEST',
        effectClasses: ['authority-epoch-transition', 'completion-revalidation', 'rollback-snapshot', 'transport-invalidation'],
        createdAt: new Date(Date.now() - 120_000).toISOString(), expiresAt,
      });
      const evidence = await fixture.harness.evidenceStore.put(resolution, {
        mediaType: recoveryResolutionMediaType, projectId: fixture.projectId, runId: 'run', epoch: state.epoch, generation: state.generation,
        sourceDigest: state.sourceDigest, artifactDigest: state.artifactDigest, policyDigest: state.policyDigest, pluginSetDigest: state.pluginSetDigest,
        labels: ['recovery-resolution'],
      });
      await assert.rejects(() => fixture.harness.recovery.hardRecover({ projectId: fixture.projectId, runId: 'run', verificationRef: verified.verification.verificationRef, resolutionRef: evidence.ref }, { expectedRevision: state.revision, commandId: `recover-${suffix}` }), error => error.code === code);
    });
  }
});
