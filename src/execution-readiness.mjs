...
export const sealExecutionReadinessReport = input => {
  ...
  assertJsonSchema(report, schema, { code: 'EXECUTION_READINESS_REPORT_INVALID', label: 'Execution Readiness Report' });
  return report;
};

export const verifyExecutionReadinessReport = (input, { plan, now = () => new Date().toISOString(), requireReady = true } = {}) => {
  assertJsonSchema(input, schema, { code: 'EXECUTION_READINESS_REPORT_INVALID', label: 'Execution Readiness Report' });
