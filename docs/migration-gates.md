...
V1.0.0 将“仓库抽离就绪”“发布候选就绪”和“生产切换就绪”分开。仓库抽离只判断独立 Harness 是否已经不依赖旧源码并具备自举能力；抽离后的 Recovery 完整性问题现已关闭，仍须形成远端发布证据，再执行真实 Capsule 和现场切换。旧内容处置始终是用户单独决定。详细台账见[仓库抽离后修复与验证台账](post-extraction-register.md)。
...
当前 RCV Recovery 完整性 Gate 已通过本地测试关闭；许可证为 `UNLICENSED / all rights reserved`。提交 `1961e578...` 及其 CardWorld/Collection 外部 clean-start Canary 是 2026-09-15 可见 Runtime 修订之前的历史证据。修订后的交互式默认 Runtime、Descriptor、Skills 和制品摘要已变化，当前为 Migration Code Ready 候选；必须基于新提交完成候选 tarball、宿主可见子 Agent Canary、外部 Authority 和零业务写入验证后，才能恢复 Local Clean-start Cutover Ready。远端 Node 22 双平台 CI、签名、tag、Release 和 Registry 发布按项目所有者决定延期。两个冻结的旧状态根均不存在，原 G7 hard recovery 与现场 rollback 不可补做；两份 source-unavailable Receipt 继续强制只允许 clean-start。
