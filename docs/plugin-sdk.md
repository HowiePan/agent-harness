...
Profile、一个或多个插件工厂、声明式 `commandManifest`、Lifecycle Plan compiler、Legacy Importer 与构建期 operation 通过 Extension Pack 组合。工具适配器先从外部安装数据解析 `h:<项目别名> <动作> <目标> [预设]` 的项目别名，随后由 `commandManifest` 把通用动作解析到该 Profile 的阶段范围和变更属性；Core 再调用 Extension 的纯 `createLifecyclePlan` compiler 生成确定性的 `LifecycleCommandPlan`。Core 不保存项目别名或项目命令表。`createHarness()` 默认只加载中立 Feature Profile 和参考插件；任何消费者、Runtime 供应商或 Legacy 能力都必须通过 Extension Registry 安装或由 API 显式传入。Project Descriptor 的 Extension ID、版本或摘要不满足时，Plan/Run 在写入 Authority 前失败。
...
生产 Project Descriptor 必须显式声明 `policy.agentExecutionMode` 与 `policy.promptCodecPlugin`。`policy.actionExecution.<action>` 可对单个 lifecycle 动作覆盖执行模式与 Runtime，但它只是 allow-policy，schema 明确禁止持久 execution authorization。Core 按当前 Command Intent 解析策略，并把模式与 Runtime 同时钉入 Lifecycle Plan、Authority 和 Dispatch，编译器、Coordinator 与 Receipt 都不能替换。每个 Dispatch 固定 Prompt Codec ID/版本和 Prompt Contract 版本；Codec 从不可变 Packet 确定性生成完整 Prompt，并产生 `packetDigest` 与 `promptDigest`。Operator/Runtime 只能原样传输该文本，禁止临时组织、改写、增删或翻译。`conversation-visible` Runtime 必须声明 `user-visible` 与 `host-orchestrated`、申请 `agent.conversation`、禁止 `process.spawn`，并通过按 Runtime ID 绑定的可信宿主适配器把 Lease Receipt 的 `agentId`、`dispatchId`、两个摘要、Prompt 身份、`visibility.mode`、surface 和 inspect reference 共同证明；提交要求新鲜 heartbeat。参考 `createVisibleHostAdapter()` 的必需 capability 集为 inspect/spawn/wait/result/reconcile/confirm/contain，可选 send/interrupt；preflight 还必须验证版本化 native Host Contract。`executeVisibleLifecyclePlan()` 负责连续协调、在任何新 spawn 前收容未绑定的失败 Agent，并在重启后重新 attestation active Lease。Core 的宿主结果通过 `visible-agent-result.schema.json`；Codex 原生输出在 Codex 适配器边界通过 `codex-runtime-result.schema.json`。repair completion 还必须包含通过且带证据的 checkpoint 与宿主保存的 verification Receipt。没有任一能力时 action preflight 必须 fail-closed。进程型 Agent Runtime 必须声明 `headless`，并同时通过 Descriptor allow-policy、可信 deny-wins 用户约束和宿主签发的 command-scoped `LifecycleExecutionGrant`；Plugin Host 与 Runtime spawn 还要求 Core 私有 launch capability。Prompt 生成或宿主不可用时禁止即兴拼 Prompt、选择 CLI Runtime 或用原始 Receipt 兜底。接入细节见 [可见 Agent 宿主接入合同](host-integration.md)。
