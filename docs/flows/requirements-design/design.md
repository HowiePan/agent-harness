...
**Workflow：** `requirements-design@1.0.0`；**Profile：** `composable-workflow`；**动作：** `analyze`。适用于多份需求文档与多个代码仓共同分析；不替代业务方对需求正确性的确认。

## 节点和结果

```text
每份文档 ingest → normalize → 每个仓库 search → map
                                            ├→ requirements-doc ┐
                                            └→ design-doc       ┴→ review
```

`ingest` 返回带来源位置的 `source-facts-v1`；`normalize` 返回 `requirements-v1`；`search` 返回 `code-evidence-v1`；`map` 返回 `impact-map-v1`。两份文档节点分别写入 Workspace 允许的输出路径并返回 `document-ref-v1`。`review` 检查两份文档和固定来源，返回通过的 `document-review-v1` 与 `memory-candidates-v1`；它不能绕过文档路径、证据或 P0–P3 质量规则。合同在 `contracts/`，节点按 `intake/`、`discovery/`、`documents/`、`review/` 阶段分组。
