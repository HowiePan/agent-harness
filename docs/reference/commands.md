...
项目初始化默认读取项目仓内 `harness.json`。`init plan` 只读，`init apply` 应用固定计划；`init execute` 在同一进程顺序完成两者，但仍要求外部 `--decision`、唯一 `--command-id` 和当前 Registry 修订。`dev execute` 直接绑定准确的 Harness source checkout，生成 development source manifest 和 Runtime generation。后续只读 `workflow list`、`lifecycle plan/preflight` 可带 `--development-manifest <json>` 使用该绑定；它不会启用 CLI Agent 执行。源码变化后先用 `dev patch status/plan` 分类：H0/H1 可按规则继续当前 Run，H2/H3 要求新 Run，H4 禁止热应用。详细命令、等级与回滚语义见[完整配置 API](./configuration-api.md#8-本地源码调试与热更新)。
