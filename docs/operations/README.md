...
本地代码准出命令：`npm test`、`npm run test:conformance`、`npm run check`、`npm run workspace:canary`、`npm run check:clean-room`、`npm run pack:dry-run`、`npm run check:residue`。四条流程必须在命令级 Canary 中各有 `closed` Run；单元测试全绿不能替代。
