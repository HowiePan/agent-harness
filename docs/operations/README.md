...
本地代码准出命令：`npm test`、`npm run test:conformance`、`npm run check`、`npm run workspace:canary`、`npm run check:clean-room`、`npm run pack:core`、`npm run pack:codex`、对两个新包运行 `npm run check:channels`、`npm run check:residue`。四条流程必须在命令级 Canary 中各有 `closed` Run；单元测试全绿不能替代。Core 与 Codex 渠道产物及本机安装入口见[渠道打包](../reference/packaging.md)。
