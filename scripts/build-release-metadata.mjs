...
const generatedAt = process.env.SOURCE_DATE_EPOCH
  ...
  : packageJson.releaseMetadata?.createdAt;
if (!generatedAt || Number.isNaN(Date.parse(generatedAt))) throw new Error('package.json releaseMetadata.createdAt or SOURCE_DATE_EPOCH is required for reproducible release metadata.');
const manifest = { protocolVersion: '1.0', name: packageJson.name, version: packageJson.version, node: packageJson.engines.node, license: packageJson.license, files, packageDigest: digestJson(files), metadataFiles: ['release-manifest.json', 'sbom.spdx.json'], generatedAt };
