...
Every production Project Descriptor explicitly declares `policy.agentExecutionMode`; schema and Registry validation reject an omitted policy. `conversation-visible` is the interactive default: the selected Agent Runtime declares `user-visible` and `host-orchestrated`, has no `process.spawn` permission, and binds each Lease through a trusted host adapter. Its receipt binds `agentId`, `dispatchId`, and `packetDigest`, contains `visibility.mode: user-visible`, a surface kind, and an inspectable task reference, and carries matching host attestation. Host-orchestrated visible Runtimes are driven by the current conversation through lifecycle start, Dispatch scheduling, native child-Agent delegation, heartbeat, bind, and submit operations. Direct execution-plugin/Kernel bypasses are denied; missing host delegation, attestation, inspection, or fresh heartbeat is attention-required and cannot select a different Runtime. `headless` is an explicit non-interactive mode that is never inferred; only Runtimes declaring `headless` may run there, and any process-capable Runtime additionally follows managed-output and sandbox requirements. Process Gates require a live progress observer before launch.
