...
6. For a state-changing intent, call the packaged `lifecycle plan` then `lifecycle execute` entrypoints with the exact current revision and a fresh command ID. The verified `LifecycleCommandPlan` owns deterministic Run ID, Feature graph, Profile config, Runtime, Gate plan, protected-operation list, and stop condition; the conversational model must not construct or patch any of those fields. Follow `$agent-harness-operator` until the Plan's scoped stop condition. The Project Descriptor must explicitly allow `workspace.rootSelector=git-worktree` before a linked worktree can be selected. For a read-only intent, do not create a Run, schedule work, run a Gate, or write Authority.
