...
For `conversation-visible` execution, use `lifecycle start`, then schedule eligible Dispatches. For every Dispatch, call the policy-aware packet read operation and take its generated `prompt.text`, `promptDigest`, `packetDigest`, Prompt Codec identity, and Prompt Contract version. Pass `prompt.text` byte-for-byte as the complete user prompt to the host's native multi-agent delegation capability; do not summarize, prepend, append, translate, repair, interpolate, or compose substitute prose. The operator is a transport for the generated Prompt, not its author. Create a visible child Agent and immediately show the Feature, visible task identity, and initial status in the current conversation. The Codex host integration must use `createCodexVisibleHostAdapter({ inspectVisibleAgent })` and pass it to `createHarness({ agentAdapter })`; this adapter must re-observe the native task at Lease binding and every heartbeat. Bind the Lease only through the trusted host adapter, never through raw CLI JSON or direct Kernel/PluginHost calls. The Runtime Receipt must bind the exact `agentId`, `dispatchId`, `packetDigest`, `promptDigest`, Prompt Codec ID/version, and Prompt Contract version, include `visibility.mode: user-visible`, its surface kind, and an inspectable task reference, and receive a matching host attestation over both packet and Prompt digests. While it runs, wait through the host, relay meaningful progress in the conversation, and record Harness heartbeats through the policy-aware `recordHeartbeat` operation; at least one fresh heartbeat is required before submission. Submit the structured result and Evidence, then continue scheduling. If generated Prompt compilation, native visible delegation, trusted host attestation, inspection, or heartbeat recording is unavailable, stop as `attention-required`; never improvise a prompt, call `codex exec`, another Agent CLI, a hidden process, `lifecycle execute`, `run execute`, or create an unrelated standalone task as a fallback.
