...
5. Read the current action's `policy.actionExecution.<action>` when present, otherwise `policy.agentExecutionMode`; also read `policy.promptCodecPlugin` and the selected Runtime and Prompt Codec manifests. Require action-scoped headless policy to contain a persisted approval bound to that same action, and require every Plan/Dispatch to pin the selected execution mode, Runtime, Prompt Codec ID, plugin version, and Prompt Contract version. In an interactive Codex task, a `conversation-visible` selection requires the Runtime to declare both `user-visible` and `host-orchestrated`, with no `process.spawn` permission. Never infer `headless` from a Runtime ID, missing adapter, CLI context, or automation-like wording; it requires an explicit user request for CI/unattended execution and an explicit Descriptor value for the current action.
