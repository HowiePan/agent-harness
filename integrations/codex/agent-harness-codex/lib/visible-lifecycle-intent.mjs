...
export const validateVisibleLifecycleIntent = (input, { now = () => new Date().toISOString() } = {}) => {
  ...
  const harnessKeys = ['controlRoot', 'dataRoot', 'entrypoint', 'coordinatorEntrypoint', 'release'];
  ...
  if (!intent.project || Object.keys(intent.project).some(key => !['projectId', 'profileId', 'extensionId'].includes(key))) throw Object.assign(new Error('Visible lifecycle Project binding is invalid.'), { code: 'VISIBLE_LIFECYCLE_PROJECT_BINDING_INVALID' });
