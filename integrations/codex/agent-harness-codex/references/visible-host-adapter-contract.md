...
Coordinator 必须由宿主直接以 `tty:true` 启动，并在不超过一秒的首次 yield 后返回可轮询的 live process session；禁止嵌套 shell、解析器、管道或阻塞式输出捕获。未获得 live session 时以 `CODEX_VISIBLE_COORDINATOR_SESSION_REQUIRED` 停止，不能转而扫描 pending 文件。planned/preflight 进度只输出有界摘要，避免 Plan 或 Readiness 明细遮蔽随后的 Host 请求。未解决的同一请求使用不变的 `requestId` 与 `requestDigest` 周期重发；宿主按这两个字段去重、只调用一次原生工具、先处理请求再等待同一 session，且不得为同一 intent 启动第二个 Coordinator。
...
`PostToolUse` Hook 的静态 bootstrap 闭包必须完全位于插件包内；实际 Host exchange 实现只能从 active Runtime composition 中经过逐文件验证的绝对模块路径动态加载，禁止依赖插件缓存之外的相对源码路径。每次调用都在 `dataRoot/diagnostics/codex-host-hook/<invocationId>/` 写入不可变、递增的阶段回执，至少覆盖 `hook-entered`、`event-parsed`、`bindings-resolved`、`bridge-module-loaded`、pending match、response commit 与 `completed|failed`。诊断只记录摘要、阶段和错误码，不保存原始工具响应。安装前的渠道组合检查必须从实际 Codex 包加载该 Hook，并完成一次合成原生事件到 Host 响应的端到端往返；单纯“模块能 import”不构成准出。
