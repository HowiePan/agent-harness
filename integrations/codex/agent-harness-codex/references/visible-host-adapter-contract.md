...
`conversation-visible` 运行必须由当前 Codex 宿主提供完整的可信协调能力。宿主集成使用 `createCodexVisibleHostAdapter({ inspectVisibleAgent, spawnVisibleAgent, waitVisibleAgent, readVisibleResult, reconcileVisibleHostEffects, confirmVisibleLease, containVisibleAgent, sendVisibleAgent?, interruptVisibleAgent? })`，再把返回值作为 `agentAdapter` 传给 `createHarness()`；不得把模型输出、原始 CLI Receipt、文件参数或用户提供的 JSON 当作证明。

Codex 插件通过 active Release manifest 中逐文件验证的 `visible-lifecycle-coordinator.mjs` 接入当前任务的 native collaboration tree。Hook 生成最长十分钟、内容摘要绑定的 `codex-visible-lifecycle-intent`；Coordinator 必须在同一长驻进程内重新验证 active Release、生成 Plan、完成 preflight 并执行可见生命周期。Host 请求和响应以单请求串行协议传输，每个响应必须逐字段匹配 session、request、request digest、operation 和 tool，且只能封装刚刚返回的原生 collaboration tool result；跨请求重放、额外字段、缺字段、任务 ID 漂移均拒绝。Native Contract `codex-collaboration-native@1.1.0` 要求 `spawn_agent` 精确返回 `{ agent_id, nickname }`，再用 `list_agents` 的 `{ agents: [{ agent_name, agent_status }] }` 唯一解析 canonical `agent_name`；provider identity 与可见任务 identity 不得混用。`wait_agent` 精确返回 `{ message, timed_out }`。`spawn_agent` 固定 `fork_turns=none`，禁止从父对话隐式继承提示上下文。
